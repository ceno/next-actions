...
import { DEFAULT_SETTINGS, LABELS, SETTINGS_KEY } from './constants';
import {
  aggregateSource,
  boardChecklistsUrl,
  clientSource,
...
export async function cardBadges(t: TrelloT): Promise<Badge[]> {
  ...
  if (isUnavailable(checklists)) {
    ...
    if (CONFIG.degradeToAggregate) {
      ...
      if (!isUnavailable(fallback)) return computeBadges(fallback, settings);
    }
    if (CONFIG.showUnauthorizedBadge && CONFIG.dataPath === 'rest') {
      const authorized = await t.getRestApi().isAuthorized().catch(() => false);
      if (!authorized) return [{ text: LABELS.unauthorized, color: 'light-gray' }];
    }
    return [];
  }

  return computeBadges(checklists, settings);
}
...
export function initialize(): void {
  powerUp().initialize({
    ...
    'show-settings': (t: TrelloT) =>
      t.popup({ title: 'Next Actions', url: './settings.html', height: 300 }),
    ...
    'show-authorization': (t: TrelloT) =>
      t.popup({ title: LABELS.unauthorized, url: './authorize.html', height: 200 }),
  });
}
